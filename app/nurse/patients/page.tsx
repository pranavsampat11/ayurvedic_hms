...
export default function NurseAllPatientsPage() {
  ...
  const [filterType, setFilterType] = useState(""); // OPD/IPD
  ...
  // Server-side pagination when filters are empty
  ...
  useEffect(() => {
    ...
    loadServerPage();
  }, [canUseServerPagination, currentPage, patientsPerPage, sortKey, sortDir]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, fromDate, toDate, filterStatus, filterGender, filterType]);
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-4 mb-4 items-end">
        ...
        <Button 
          variant="outline" 
          onClick={() => {
            setSearch(""); setFromDate(""); setToDate(""); setFilterStatus(""); setFilterGender(""); setFilterType(""); setShowTodaysAppointments(false); setPatients(allPatients);
          }}
          className="text-sm"
        >
          Clear Filters
          ...
          <Table>
            ...
            <TableBody>
              {currentPatients.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={10} className="text-center py-12 text-slate-500">No patients found</TableCell>
                </TableRow>
              ) : (
