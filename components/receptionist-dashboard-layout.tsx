...
export default function ReceptionistDashboardLayout({ children, title }: ReceptionistDashboardLayoutProps) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      setReceptionistName(localStorage.getItem("userName"));
    }
  }, []);
      ...
      <div className={cn(
        ...
        <div className="p-4 md:p-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-white">Receptionist</h1>
        ...
        <div className="mt-auto p-4 border-t border-slate-700">
          <div className="flex items-center gap-3">
            <Avatar className="h-8 w-8">
              <AvatarFallback className="bg-slate-600 text-white">{receptionistName ? receptionistName[0] : "R"}</AvatarFallback>
            </Avatar>
            <div className="grid gap-0.5">
              <div className="font-semibold text-sm capitalize text-white">Receptionist</div>
              <div className="text-xs text-slate-300">{receptionistName}</div>
            </div>
          </div>
          <Button 
            variant="ghost" 
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <header className="md:hidden flex h-14 items-center gap-4 border-b bg-white px-4 shadow-sm">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-semibold capitalize">{title || "Receptionist Dashboard"}</h1>
            <p className="text-xs text-muted-foreground capitalize">{receptionistName}</p>
          </div>
        </header>

        {/* Desktop Header */}
        <header className="hidden md:flex h-14 items-center gap-4 border-b bg-muted/40 px-4 lg:h-[60px] lg:px-6">
          <h1 className="text-lg font-semibold md:text-2xl capitalize">{title || "Receptionist Dashboard"}</h1>
          <div className="ml-auto flex items-center gap-2">
            <span className="text-sm text-muted-foreground capitalize">{receptionistName}</span>
          </div>
        </header>
